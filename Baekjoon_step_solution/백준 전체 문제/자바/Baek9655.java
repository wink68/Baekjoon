...
public class Baek9655 {
	...
	public static void main(String[] args) throws IOException {
		...
		// 상근이가 먼저 뽑을 때, 마지막에 돌을 집어가는 사람이 승리
		// 뽑는 횟수가 홀수면 상근(SK)이 승, 짝수면 창영(CY) 승
		if (N % 3 == 0) {                           // 돌이 3의 배수일 때
			if ((N / 3) % 2 == 0) {             // 2, 4... 짝수번째 가져가는 상근이 승
		...
		// 돌의 개수가 3의 배수가 아닌 경우           // 손 코딩 해보기
			...
			if ((N % 3) % 2 == 0) {             // 3으로 뽑은 횟수 짝수 + 나머지 횟수 짝수
				System.out.println("CY");
			} else {
				System.out.println("SK");
			}
			...
			if ((N % 3) % 2 != 0) {             // 3으로 뽑은 횟수 홀수 + 나머지 횟수 홀수
				System.out.println("CY");
			} else {
				System.out.println("SK");
			}
